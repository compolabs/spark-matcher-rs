...
impl SparkMatcher {
    ...
    debug!("Sell orders: {:?}", sell_orders.iter().take(5).collect::<Vec<_>>());
    debug!("Buy orders: {:?}", buy_orders.iter().take(5).collect::<Vec<_>>());
    ...
    while sell_index < sell_orders.len() && buy_index < buy_orders.len() {
        ...
        let (sell_size, sell_price, buy_size, buy_price) = (
            sell_order.base_size.parse::<i128>().context("Invalid sell order size")?,
            sell_order.base_price.parse::<i128>().context("Invalid sell order price")?,
            buy_order.base_size.parse::<i128>().context("Invalid buy order size")?,
            buy_order.base_price.parse::<i128>().context("Invalid buy order price")?,
        );

        debug!("Matching sell order: {:?}, buy order: {:?}", sell_order, buy_order);
